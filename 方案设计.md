...
- `GET /api/events?uid={uid}`
  - SSE 事件流。
  - 捕获相关事件：`type = pet_info.changed`。
  - 精灵类型 ID：`data.base_conf_id`。
  - 单只精灵唯一 ID：`data.gid`。
...
3. 创建 SSE 连接。
4. 收到事件后解析 JSON。
5. 如果 `type != pet_info.changed`，忽略。
6. 读取 `data.base_conf_id` 和 `data.gid`。
7. 如果 `base_conf_id` 不在当前目标 ID 集合，忽略。
8. 如果 `gid` 已存在于 `caughtGids`，忽略。
9. 将 `gid` 加入去重集合。
10. 记录捕获事件，更新计数、速率和历史曲线。
11. 如果计数达到目标并且 `targetNotifySent=false`，发送目标达成通知并标记已发送。
12. 如果计数已达到目标，低速提醒停止判断。
13. 如果计数未达到目标，继续执行低速提醒状态机。
...
当前速率按最近 60 秒有效运行窗口计算。

推荐实现：

- 每次捕获事件记录 `effectiveRunMillis`。
- 当前速率统计 `effectiveRunMillis >= currentEffectiveRunMillis - 60000` 的捕获数。
- 换算为每分钟：

```text
currentRate = countInLastEffective60Seconds
```
...
```text
bucketIndex = effectiveRunMillis / 60000
```
...
- 前台服务启动时必须有前台服务通知。
- Android 14/API 34+ 前台服务需要声明合适类型，本项目建议使用 `dataSync`，并声明 `FOREGROUND_SERVICE_DATA_SYNC`。
...
data class SseEnvelope(
    val type: String,
    val data: PetChangedData?
)

data class PetChangedData(
...
MVP 可以只持久化设置，不持久化运行中任务。原因：

- App 被系统杀死后，SSE 连接必然断开。
- 恢复任务需要确认洛克助手当前连接、用户和目标，自动恢复容易造成误判。
